var counter = 0;
var button  = document.getElementById("r" + counter);

while ( button )
{
   button.addEventListener("click", function(event){
       var x = event.target.innerHTML;
       //var x = button.innerHTML;
       if(x==0){
           x = 1;
       }else{
           x = 0;
       }
       event.target.innerHTML = x;
       //document.getElementById("r0").innerHTML=x;
   });
   button  = document.getElementById( "r" + ( ++counter ));
}

/*
document.getElementById("r0").onclick = function(){
    var x = document.getElementById("r0").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r0").innerHTML=x;
}
document.getElementById("r1").onclick = function(){
    var x = document.getElementById("r1").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r1").innerHTML=x;
}
document.getElementById("r2").onclick = function(){
    var x = document.getElementById("r2").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r2").innerHTML=x;
}
document.getElementById("r3").onclick = function(){
    var x = document.getElementById("r3").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r3").innerHTML=x;
}
document.getElementById("r4").onclick = function(){
    var x = document.getElementById("r4").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r4").innerHTML=x;
}
document.getElementById("r5").onclick = function(){
    var x = document.getElementById("r5").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r5").innerHTML=x;
}
document.getElementById("r6").onclick = function(){
    var x = document.getElementById("r6").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r6").innerHTML=x;
}
document.getElementById("r7").onclick = function(){
    var x = document.getElementById("r7").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r7").innerHTML=x;
}
document.getElementById("r8").onclick = function(){
    var x = document.getElementById("r8").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r8").innerHTML=x;
}
document.getElementById("r9").onclick = function(){
    var x = document.getElementById("r9").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r9").innerHTML=x;
}
document.getElementById("r10").onclick = function(){
    var x = document.getElementById("r10").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r10").innerHTML=x;
}
document.getElementById("r11").onclick = function(){
    var x = document.getElementById("r11").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r11").innerHTML=x;
}
document.getElementById("r12").onclick = function(){
    var x = document.getElementById("r12").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r12").innerHTML=x;
}
document.getElementById("r13").onclick = function(){
    var x = document.getElementById("r13").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r13").innerHTML=x;
}
document.getElementById("r14").onclick = function(){
    var x = document.getElementById("r14").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r14").innerHTML=x;
}
document.getElementById("r15").onclick = function(){
    var x = document.getElementById("r15").innerHTML;
    if(x==0){
        x = 1;
    }else{
        x = 0;
    }
    document.getElementById("r15").innerHTML=x;
}
*/
